...
// stopCheck 停止判断
func stopCheck(cfg *config.Config, current int) bool {
	if cfg.Stream {
...
func nonStreamStopCheck(cfg *config.Config, current int) bool {
	...
	if successRate < 0.95 {
		...
		if ok {
			log.Infof("Max throughput %.3f tokens/s, %.3f req/s, prompt length: %v",
				statistics[last].OutputTokensPerSecond, statistics[last].RequestPerSecond, cfg.InputTokens)
		} else {
	...
	// 当前吞吐量
	currentThroughput := statistics[current].OutputTokensPerSecond
	var pastThroughput float64 = 0
	for i := 1; i <= window; i++ {
		pastThroughput += statistics[current-i*increment].OutputTokensPerSecond
	}
	historyAvg := pastThroughput / float64(window)

	// 吞吐量收敛了，停止
	if common.IsClose(currentThroughput, historyAvg, 0.02) {
